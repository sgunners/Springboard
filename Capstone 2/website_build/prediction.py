...
#twitter keys: add appropriate as needed
consumer_key = 
consumer_secret = 
access_token = 
access_token_secret = 
...
def final_tweet_model(test_text):
    ...
    if value<0.5:
        ...
        best_genre='Non-fiction'
        best_tweet=''
        test_tweets = cmu_summ_full['FicnonFic']['count_vect'].transform(outtweets['c_tokens'])
        test_proba = cmu_summ_full['FicnonFic']['count_model'].predict_proba(test_tweets)
        best_value = max(list(next(zip(*test_proba))))
        outtweets['pred_prob']=pd.Series(list(next(zip(*test_proba))))
        best_tweet = outtweets[outtweets['pred_prob']==best_value].reset_index()
    final_best = fo[fo[0]==best_tweet[0][0]].reset_index()
    return value, df, wordcloud, best_value, final_best
